...
public class HomeDataSource extends PageKeyedDataSource<Integer, Datum> {
    ...
    public HomeDataSource(String id,String token,Boolean isLogin) {
        this.id = id;
        this.token = token;
    ...
    public void loadInitial(@NonNull LoadInitialParams<Integer> params,@NonNull LoadInitialCallback<Integer, Datum> callback) {
        ...
        Log.e("loginStatus",String.valueOf(isLogin));
        Observable observable;
        Observer<HomeModel> observer;
        if (isLogin) {
            observable = RetroWeb.getClient().create(MainServices.class).home(token,id,FIRST_PAGE)
            ...
            observable = RetroWeb.getClient().create(MainServices.class).homeVisitor(id,FIRST_PAGE)
                    .subscribeOn(Schedulers.io())
                    .observeOn(AndroidSchedulers.mainThread());
            observer = new Observer<HomeModel>() {
                @Override
    ...
    public void loadBefore(@NonNull LoadParams<Integer> params,@NonNull LoadCallback<Integer, Datum> callback) {
        _isViewLoading.postValue(true);
        Log.e("loginStatus1",String.valueOf(isLogin));
        Observable observable;
        Observer<HomeModel> observer;
        if (isLogin) {
            observable = RetroWeb.getClient().create(MainServices.class).home(token,id,FIRST_PAGE)
    ...
    public void loadAfter(@NonNull LoadParams<Integer> params,@NonNull LoadCallback<Integer, Datum> callback) {
        _isViewLoading.postValue(true);
        Log.e("loginStatus2",String.valueOf(isLogin));
        Observable observable;
        Observer<HomeModel> observer;
        if (isLogin) {
            observable = RetroWeb.getClient().create(MainServices.class).home(token,id,params.key)
                    .subscribeOn(Schedulers.io())
                    .observeOn(AndroidSchedulers.mainThread());
            observer = new Observer<HomeModel>() {
                @Override
            ...
            observable = RetroWeb.getClient().create(MainServices.class).homeVisitor(id,params.key)
                    .subscribeOn(Schedulers.io())
                    .observeOn(AndroidSchedulers.mainThread());
            observer = new Observer<HomeModel>() {
                @Override
