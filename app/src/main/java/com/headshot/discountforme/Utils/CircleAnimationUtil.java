...
public class CircleAnimationUtil {
    ...
    private boolean prepare() {
        if (mContextReference.get() != null) {
            ...
            mContextReference.get().getWindowManager().getDefaultDisplay().getMetrics(displayMetrics);
//            mTarget.setLayoutDirection(View.LAYOUT_DIRECTION_LTR);
            int widthPixels = displayMetrics.widthPixels;
            Log.i("screen_widd", "" + widthPixels);
            ...
            Log.i("screen_", "prepare: " + src[0]);
//            params.setMargins(widthPixels/2, src[1], widthPixels, 0);

            // left top right bottom

            String lang = mContextReference.get().getSharedPreferences("lang", Context.MODE_PRIVATE).getString("mlang", Locale.getDefault().getLanguage());
            Log.i("lang_an", Locale.getDefault().getLanguage());
            if (lang.equals("AR") || lang.equals("ar")) {


                params.setMargins(src[0], src[1], 0, 0);
//            if (src[0]==0){
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
                    params.setMarginStart((widthPixels / 2) - src[0]);
                }
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
