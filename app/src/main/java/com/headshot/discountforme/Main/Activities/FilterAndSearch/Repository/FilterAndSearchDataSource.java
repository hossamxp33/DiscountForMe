...
public class FilterAndSearchDataSource extends PageKeyedDataSource<Integer, Datum> {
    ...
    public FilterAndSearchDataSource(String token,String categories,String order_by,Boolean isLogin) {
        this.token = token;
    ...
    public void loadInitial(@NonNull LoadInitialParams<Integer> params,@NonNull LoadInitialCallback<Integer, Datum> callback) {
        ...
        if (isLogin) {
            observable = RetroWeb.getClient().create(MainServices.class).filter(token,categories,order_by,FIRST_PAGE)
            ...
            observable = RetroWeb.getClient().create(MainServices.class).filterVisitor(categories,order_by,FIRST_PAGE)
                    .subscribeOn(Schedulers.io())
                    .observeOn(AndroidSchedulers.mainThread());
            observer = new Observer<HomeModel>() {
                @Override
    ...
    public void loadAfter(@NonNull LoadParams<Integer> params,@NonNull LoadCallback<Integer, Datum> callback) {
        ...
        Observer<HomeModel> observer;
        if (isLogin) {
            observable = RetroWeb.getClient().create(MainServices.class).filter(token,categories,
                    order_by,params.key)
                    .subscribeOn(Schedulers.io())
                    .observeOn(AndroidSchedulers.mainThread());
            observer = new Observer<HomeModel>() {
            ...
            observable = RetroWeb.getClient().create(MainServices.class).filterVisitor(categories,
                    order_by,params.key)
                    .subscribeOn(Schedulers.io())
                    .observeOn(AndroidSchedulers.mainThread());
            observer = new Observer<HomeModel>() {
