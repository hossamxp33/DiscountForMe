...
public class SearchDataSource extends PageKeyedDataSource<Integer, Datum> {
    ...
    public SearchDataSource(String token, String search) {
        this.token = token;
        this.search = search;
    ...
    public void loadInitial(@NonNull LoadInitialParams<Integer> params, @NonNull LoadInitialCallback<Integer, Datum> callback) {
        ...
        Observable observable = RetroWeb.getClient().create(MainServices.class).search(token, search, FIRST_PAGE)
    ...
    public void loadAfter(@NonNull LoadParams<Integer> params, @NonNull LoadCallback<Integer, Datum> callback) {
        ...
        Observable observable = RetroWeb.getClient().create(MainServices.class).search(token, search, params.key)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread());
        Observer<HomeModel> observer = new Observer<HomeModel>() {
            @Override
