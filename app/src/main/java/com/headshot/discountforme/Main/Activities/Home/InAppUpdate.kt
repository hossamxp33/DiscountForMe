package com.headshot.discountforme.Main.Activities.Home
...
class InAppUpdate(activity: Activity) : InstallStateUpdatedListener {
    ...
    init {
        Toast.makeText(activity,"sad",Toast.LENGTH_SHORT)

        appUpdateManager = AppUpdateManagerFactory.create(parentActivity)
        appUpdateManager.appUpdateInfo.addOnSuccessListener { info ->
            // Check if update is available
            Toast.makeText(activity,"sad",Toast.LENGTH_SHORT)
            Log.e("lang", info.updateAvailability().toString())
            ...
            if (info.updateAvailability() == UpdateAvailability.UPDATE_AVAILABLE) { // UPDATE IS AVAILABLE
                if (info.updatePriority() == 5) { // Priority: 5 (Immediate update flow)
                    ...
                    Toast.makeText(activity,"sadddd",Toast.LENGTH_SHORT)
    ...
    private fun flexibleUpdateDownloadCompleted() {
        Snackbar.make(
            parentActivity.findViewById(R.id.home),
