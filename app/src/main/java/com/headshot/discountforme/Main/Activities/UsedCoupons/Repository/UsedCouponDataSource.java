...
public class UsedCouponDataSource  extends PageKeyedDataSource<Integer, Datum> {
    ...
    public UsedCouponDataSource(String token) {
        this.token = token;
    ...
    public void loadInitial(@NonNull LoadInitialParams<Integer> params, @NonNull LoadInitialCallback<Integer, Datum> callback) {
        ...
        Observable observable = RetroWeb.getClient().create(MainServices.class).used_coupons(token, FIRST_PAGE)
    ...
    public void loadAfter(@NonNull LoadParams<Integer> params, @NonNull LoadCallback<Integer, Datum> callback) {
        ...
        Observable observable = RetroWeb.getClient().create(MainServices.class).used_coupons(token, params.key)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread());
        Observer<FavouritesModel> observer = new Observer<FavouritesModel>() {
            @Override
